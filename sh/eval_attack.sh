...
# Evaluate attack results using the refactored RedEval CLI
...
# Configuration
BASE_LOG_DIR="${REDEVAL_LOG_DIR}/attack"
CONFIG_PATH="${REDEVAL_RECIPES_DIR}/attack/eval.yml"

# Dataset and method configurations
SUBDATASETS=${REDEVAL_ATTACK_DATASETS:-"HarmBench"}
MODEL_NAMES=${REDEVAL_MODELS:-"Qwen/Qwen2.5-7B-Instruct gpt-4o-mini"}
METHOD_NAMES=${REDEVAL_ATTACK_METHODS:-"direct human_jailbreak zeroshot"}

echo "Evaluating attack results..."
    ...
    for METHOD_NAME in $METHOD_NAMES; do
        for MODEL_NAME in $MODEL_NAMES; do
            ...
            if [ -d "$LOG_DIR" ]; then
                ...
                python -m redeval.cli eval-attack \
                    --config "$CONFIG_PATH" \
                    --log-dir "$LOG_DIR"
